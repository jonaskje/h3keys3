...
const O_UMLAUT: char = 192 as u8 as char;
...
fn altgr_key<T: VirtualKey>(kv: T) -> RemapTarget {
    RemapTarget::KeySeq(vec![
        key_down(winuser::VK_CONTROL),
        key_down(winuser::VK_MENU),
        key_down(kv),
        key_up(kv),
        key_up(winuser::VK_MENU),
...
impl InputHookState {
    ...
    fn key_hook(&mut self, code: i32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
        if winuser::HC_ACTION == code {
            ...
            if key_pressed {
                if input_key.vkCode as i32 > 100 {
                    //toast_notification(&format!("Key {}", input_key.vkCode as i32));
                }
            ...
            if key_pressed || key_released {
                ...
                let remap = if self.mod1_on {
                    ...
                    let mapped_key = match input_key.vkCode as u8 as char {
                        ...
                        'I' => key(winuser::VK_END),

                        // Undo/redo
                        'Z' => down_only(ctrl_key('Z')),
                        'Y' => down_only(ctrl_key('Y')),
                        ...
                        // VIM-like navigation
                        'H' => key(winuser::VK_LEFT),
                        'L' => key(winuser::VK_RIGHT),
                        // caps-k is up
                        // caps-ctrl-k is page up
                        'K' => {
                            if self.ctrlmod_on {
                        ...
                        // 7,8,9,0 => {, [, ], }
                        '7' => down_only(altgr_key('7')),
                        '8' => down_only(altgr_key('8')),
                        '9' => down_only(altgr_key('9')),
                        '0' => down_only(altgr_key('0')),

                        // Delete and backspace
                        'M' => key(winuser::VK_DELETE),
                        'N' => key(winuser::VK_BACK),

                        // Slashes ,. = /\
                        COMMA => down_only(shift_key('7')),
                        PERIOD => down_only(altgr_key(PLUS)),

                        O_UMLAUT => key(winuser::VK_RETURN),
